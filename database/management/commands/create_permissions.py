...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        available_leave_ct: ContentType = ContentType.objects.get_for_model(
            AvailableLeave
        )
        all_permss = Permission.objects.filter(
            content_type__in=[leave_ct, available_leave_ct]
        )

        # All permissions that belong to Leave (built-ins + custom)
        leave_perms = Permission.objects.filter(content_type=leave_ct)
        available_leave_perms = Permission.objects.filter(
            content_type=available_leave_ct
        )
